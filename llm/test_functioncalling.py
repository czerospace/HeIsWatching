...
# -*- coding: utf-8 -*-
"""
该脚本用于测试指定的大模型API是否支持function calling功能
通过定义几个测试函数并发送给模型调用请求，验证模型是否能正确返回函数调用信息
测试模型地址: http://192.168.101.185:8017/v1/chat/completions
测试模型名称: qwen3-1.7b

新增功能：查询模型支持的function calling相关信息
"""
...
# 模型API配置
API_URL = "http://192.168.101.185:8017/v1/chat/completions"
...
def query_model_info():
    ...
    try:
        # 尝试获取模型列表信息
        response = requests.get("http://192.168.101.185:8017/v1/models", timeout=10)
